...
2. 组件必须且只能返回一个 JSX 元素、null、可迭代对象，如果不想在最外层包裹一个 div，可以使用 <></> （因为每个JSX元素都是一个对象，而 return 语句只能接受一个值）
...
8. JSX 不会渲染 null ，如 `<h1>{null}</h1>`，检查浏览器存在H1这个元素，但不会将 null 作为文本值显示出来。
